...
void ensure_capacity(vec *v, int c) {
  if (v->sz == 0) die("realloacing vector with element size 0.");
  if (v->c >= c) return;
  if (v->c <= 0) v->c = 1;
  while (v->c < c) {
    v->c *= 2;
...
void vec_insert(vec *v, int index, void *elem) {
  if (index < 0 || index > v->n) die("vec_insert index out of range.");
  ensure_capacity(v, v->n + 1);
...
bool vec_push_slice(vec *destination, const vslice *source) {
  ...
  if (destination->sz != source->sz) return false;
...
vslice vec_slice(vec *v, int start, int end) {
  ...
  if (v) {
    ...
    if (start < 0 || end < 0 || start > end) return no_slice;
...
void vec_zero(vec *v) {
  if (v->sz == 0) die("vec_zero on zero sized vector");
...
bool vslice_contains(const vslice *v, const void *elem) {
  for (int i = 0; i < v->n; i++) {
    ...
    if (memcmp(item, elem, v->sz) == 0) return true;
  }
  return false;
}
...
void *ereallocarray(void *array, size_t nmemb, size_t size) {
  ...
  if (!(p = reallocarray(array, nmemb, size))) die("reallocarray:");
