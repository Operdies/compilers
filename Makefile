...
$(BIN_DIR)%: $(BIN_DIR)%.o $(OBJ_OUT) | $(DIRECTORIES) 
	@$(CC) $(CFLAGS) -o $@ $< $(call LINKSTUFF,$<) $(LDFLAGS)
...
# Objective: Detect object dependencies of a given object. 
# Assumptions: $(1) is a .o file, with MMD output in $(1).d
# Idea: If a header file is included, find a corresponding object file.
# This implements the idea, but: Problems:
# 1. There are implementation files without headers. It is generally hard
# to know if some implementation file implements some function from a header file.
# 2. If two different implementation files implement functions from the same header,
# there is no way to know which implementation should be linked.
# 3. Indirect dependencies are not detected. This is the nail in the coffin.
# 4. This whole thing assumes there is no *external* linkage required.
# So even if this whole thing worked, there would still be more work to do if say,
# -lmath was required.
define LINKSTUFF
$(uniq \
	$(filter $(OBJ_OUT), \
		$(patsubst %.h,%.o,\
			$(patsubst $(INCLUDE_DIR)/%,$(OBJ_OUT_DIR)/%,\
				$(filter %.h,\
					$(foreach dep,\
						$(shell cat $(1).d | tr -d \\\\ |  cut -d' ' -f 2-),$(dep)))))))
endef
