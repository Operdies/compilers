...
BENCH_OUT_DIR = $(BIN_DIR)$(BENCH_DIR)
...
BENCH_SRC = $(call rwildcard,$(BENCH_DIR),*.c)
...
BENCH_OUT  = $(patsubst $(BENCH_DIR)/%, $(BENCH_OUT_DIR)/%, $(BENCH_SRC:.c=))
BINARIES = $(TEST_OUT) $(CMD_OUT) $(BENCH_OUT)
...
$(BENCH_OUT_DIR)%.md: $(BENCH_OUT_DIR)%
	hyperfine -N --export-markdown '$@' '$^'
.PHONY: incremental-bench
incremental-bench: $(BENCH_RESULT)
...
$(foreach target,$(TEST_SRC) $(CMD_SRC) $(BENCH_SRC), $(eval $(call \
	LINK_OBJECTS,\
	$(target),\
