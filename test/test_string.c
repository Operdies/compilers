...
int test_vec(void) {
  ...
  for (int i = 0; i < LENGTH(payload); i++) vec_push(&v, &payload[i]);

  int doublings = 3;
  for (int n = 0; n < doublings; n++) {
    if (!vec_push_slice(&v, &v.slice)) return 2;
  }

  int expected_count = 24;
  vslice s = vec_slice(&v, 0, -1);
  if (s.n != expected_count) return 3;

  for (int i = 0; i < s.n; i++) {
    int expected = payload[i % LENGTH(payload)];
    int *actual = (int *)vec_nth(&s, i);
    if (!actual || expected != *actual) return 4;
  }
...
void test_vec_write(void) {
  ...
  vec_write(&v, "Hello %d %s\n", 2, "bro");
  vec_push(&v, &(char){0});
  if (strcmp("Hello 1 guy\nHello 2 bro\n", v.array) != 0) die("vec write broken");
  vec_destroy(&v);
}
...
void test_vec_insert(void) {
  ...
  if (v.n != 5) die("vec insert failed: expected 5 elements");
  v_foreach(int *, val, v) {
    if (*val != idx_val) die("vec insert failed: expected %d, got %d", idx_val, *val);
  }
  vec_clear(&v);
  ...
  v_foreach((void), val, v) {
    ...
    if (*val != (expected)) die("vec insert failed: expected %d, got %d", expected, *val);
  }
